...
`}async function B(){return N("html")}async function a(){return N("iframe")}async function P(){return N("pdf")}async function N(H="html"){var _e,oe,fe;const m=v.value;if(!m)return;k.value="processing";const O=H==="pdf"&&typeof window<"u"?window.open("","_blank"):null;if(H==="pdf"&&!O){k.value="error",setTimeout(()=>k.value="",3e3);return}const ie=H==="pdf"?null:new oi,te=ie?ie.folder("assets"):null;let ke=0;async function R(ae,ge="media"){if((!ae||ae.startsWith("data:")||ae.startsWith("http://localhost")||ae.startsWith("blob:"))&&ae.startsWith("data:"))return ae;try{const se=await(await fetch(ae)).blob();if(H==="pdf")return await U(se);let I="bin";const S=se.type;if(S.includes("image/png")?I="png":S.includes("image/jpeg")?I="jpg":S.includes("image/gif")?I="gif":S.includes("image/svg")?I="svg":S.includes("image/webp")?I="webp":S.includes("video/mp4")?I="mp4":S.includes("audio/mpeg")&&(I="mp3"),I==="bin"){const c=ae.match(/\\.([a-zA-Z0-9]+)(\\?.*)?$/);c&&(I=c[1])}ke++;const ye=`${ge}_${ke}.${I}`;return te.file(ye,se),`assets/${ye}`}catch(Me){return console.warn("Could not fetch asset:",ae,Me),ae}}const F=JSON.parse(JSON.stringify([...m.slides||[]])).sort((ae,ge)=>(ae.order??0)-(ge.order??0));if(G.value)for(const ae of F){ae.backgroundType==="image"&&ae.backgroundImage&&(ae.backgroundImage=await R(ae.backgroundImage,"bg"));for(const ge of ae.elements||[])ge.type==="image"&&((_e=ge.content)!=null&&_e.src)&&(ge.content.src=await R(ge.content.src,"img")),ge.type==="video"&&((oe=ge.content)!=null&&oe.src)&&!ge.content.src.includes("youtube")&&!ge.content.src.includes("youtu.be")&&(ge.content.src=await R(ge.content.src,"vid")),ge.type==="audio"&&((fe=ge.content)!=null&&fe.src)&&(ge.content.src=await R(ge.content.src,"aud"))}const ce=W(g.value||m.name||"presentation"),be={name:m.name,theme:m.theme||{},settings:m.settings||{},slides:F},ne=JSON.stringify(be).replace(/</g,"\\u003c").replace(/>/g,"\\u003e").replace(/&/g,"\\u0026"),we=me(m.theme,m.settings,H),C=z(H),j=`<script id="lf-data" type="application/json">${ne}<\/script>`,X=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
